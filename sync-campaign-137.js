/**
 * סקריפט לסנכרון תרומות של קמפיין 137 בלבד ל-Money
 */
...
async function syncCampaign137() {
    console.log('='.repeat(60));
    console.log('🚀 סנכרון קמפיין 137 ל-Money');
    console.log('='.repeat(60));

    // מציאת המיפוי לקמפיין 137
    ...
    for (const donation of donations) {
        // בדיקה אם יש moneyDonorId קיים לאותו person באותו קמפיין
        let existingMoneyDonorId = null;
        if (donation.donor?.personId) {
            const existingDonation = await prisma.donation.findFirst({
                where: {
                    moneyDonorId: { not: null },
                    donor: {
                        campaignId: 137,
                        personId: donation.donor.personId
                    }
                },
        ...
        if (result && result.donor_id) {
            ...
            console.log(`    ✅ נשלח, moneyDonorId: ${result.donor_id}`);
            sent++;
        } else {
            failed++;
        }
