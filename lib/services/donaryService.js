...
async function processChargeWebhook(webhookData, prisma, campaignId) {
    ...
    // Some charge payloads carry a bare "totalScheduleAmount" (no scheduleUniqueID/
    // countOfSchedules/firstPayment wrapper) to signal this charge is the first of a
    // multi-payment commitment - verified against campaign 212 (amount:1,
    // totalScheduleAmount:2, paymentNumber "15642-1" => 2 payments of 1 each). Derive
    // numberOfPayments from the ratio when it divides evenly; otherwise treat as a single
    // payment rather than guess at a fractional payment count.
    let numberOfPayments = 1;
    if (webhookData.totalScheduleAmount) {
        const total = parseFloat(webhookData.totalScheduleAmount);
        const perPayment = parseFloat(amount);
        if (total > 0 && perPayment > 0 && Number.isInteger(total / perPayment)) {
            numberOfPayments = total / perPayment;
        } else {
            console.error('[Donary Webhook] totalScheduleAmount present but not a clean multiple of amount, treating as single payment:', { totalScheduleAmount: webhookData.totalScheduleAmount, amount, paymentNumber });
        }
    }

    // Create the donation (single payment, or multi-payment if totalScheduleAmount indicated one)
