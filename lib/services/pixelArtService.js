...
export async function sendDonationToPixelArt(donationId) {
    try {
        ...
        // קמפיין "תרומה לפי יחידות": שולחים לפיקסל ארט את כמות היחידות במקום הסכום הכספי.
        // התרומה תמיד נשמרת בכסף (monthlyAmount), וכמות היחידות מחושבת: כסף / מחיר-יחידה.
        const screenSettings = await prisma.publicScreenSettings.findUnique({
            where: { campaignId: campaign.id },
            select: { unitDonationMode: true, unitPrice: true }
        });
        const unitPrice = Number(screenSettings?.unitPrice) || 0;
        const isUnitMode = !!screenSettings?.unitDonationMode && unitPrice > 0;

        // הערך ומספר התשלומים שיישלחו ל-PixelArt.
        // רגיל: הסכום הכספי לתשלום בודד + מספר התשלומים בפועל.
        // יחידות: כמות היחידות שהתורם בחר, כתשלום אחד בלבד (num_of_months = 1).
        //   הכמות שנבחרה נגזרת מהכסף: בקמפיין חודשי לפי הסכום החודשי, בגלובלי לפי הסכום הכולל.
        //   דוגמה: תרומה של 3 מטר לחודש למשך 60 תשלומים → נשלח "3 מטר × 1 תשלום".
        let payloadValue;
        let payloadMonths;
        if (isUnitMode) {
            const selectedMoney = campaign.donationType === 'monthly' ? monthly : total;
            payloadValue = selectedMoney / unitPrice;
            payloadMonths = 1;
        } else {
            payloadValue = monthly;
        ...
        const payload = {
            ...
            donation: {
                // PixelArt מצפה לערך כפול 100: 100 = ‏1.00 (בקמפיין רגיל = ש"ח, בקמפיין יחידות = יחידה אחת)
                monthly_amount: Math.round(payloadValue * 100),
                num_of_months: payloadMonths,
                ...(rankTitle && { title: rankTitle }),
                ...(donation.dedication && { dedication: donation.dedication }),
            }
        };

        console.log(`📤 Sending donation ${donation.id} to PixelArt event ${settings.eventId}${isUnitMode ? ' (unit mode)' : ''}`);
