...
export default function PublicCampaignScreen() {
        ...
        async function fetchData() {
            try {
                ...
                if (result.success) {
                    ...
                    console.log('📝 Recent donations with dedications:', result.data.recentDonations?.map(d => ({ id: d.id, dedication: d.dedication })));
                    setData(result.data);
    ...
    useEffect(() => {
        ...
        console.log('🔍 Looking for fundraiser ID:', fundraiserId);
        console.log('📋 Available fundraisers:', data.fundraisers);
        ...
        console.log('👥 Fundraiser donors:', fundraiser?.donors);
    ...
    useEffect(() => {
        ...
        async function setupPusher() {
            try {
                ...
                if (!key) {
                    console.log('Pusher not configured - using polling only');
                    return;
                }
                ...
                console.log('Pusher setup failed:', err);
            }
        }
    ...
    const formatCurrency = (value) => {
        ...
        console.log('💰 Currency from DB:', currencyFromDB, '→ Mapped to:', currency, 'Value:', value);
    ...
    const { campaign, statistics, settings, recentDonations, topDonors, ranks, publicScreenRanks, fundraisers } = data;
    const progressPercent = Math.min(statistics.progressPercentage, 100);
    
    // Debug: check banners data
    console.log('🎯 Banners data:', data.publicScreenBanners);
    ...
    console.log('🎯 publicScreenRanks:', publicScreenRanks);
    console.log('🏆 regular ranks:', ranks);
    ...
    // Debug: log final donation amounts with images
    console.log('💰 Final donation amounts to display:', donationAmounts);
    donationAmounts.forEach((rank, idx) => {
        console.log(`  ${idx + 1}. Name: ${rank.name}, Amount: ${rank.amount}, Image: ${rank.image || 'NO IMAGE'}`);
    });
    ...
    const calculateDynamicStats = () => {
        ...
        // 3. Hourly average - total donations divided by campaign hours
        const totalAmount = recentDonations.reduce((sum, donation) => sum + (donation.totalAmount || 0), 0);
        
        // Calculate campaign hours (from first donation to now)
        let campaignHours = 1;
        if (recentDonations.length > 0) {
            // Find the earliest donation
            const sortedByDate = [...recentDonations].sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
            const firstDonationDate = new Date(sortedByDate[0].createdAt);
            const diffTime = Math.abs(now - firstDonationDate);
    ...
    const getTabContent = () => {
        switch (activeTab) {
            ...
            case 'fundraisers':
                ...
                console.log('🎯 Fundraisers data:', fundraisers);
                return {
            ...
            case 'recent':
                ...
                return {
                    data: recentDonations?.slice(0, 20).map((donation, index) => ({
                        id: donation.id || index,
                        donorName: donation.donorName,
                        donorFirstName: donation.donorFirstName,
                        donorLastName: donation.donorLastName,
                    ...
                    emptyMessage: t('noRecentDonations')
                };
            default:
    ...
    return (
        <div className={styles.container} style={{ direction: locale === 'he' ? 'rtl' : 'ltr' }}>
            ...
            {console.log('🔍 Checking banners:', {
                exists: !!data.publicScreenBanners,
                length: data.publicScreenBanners?.length,
                data: data.publicScreenBanners
            })}
            {data.publicScreenBanners && data.publicScreenBanners.length > 0 ? (
                <div className={styles.bannerSection}>
                    ...
                    <img 
                        ...
                        onError={(e) => {
                            console.error('Banner image failed to load:', data.publicScreenBanners[currentBannerIndex]);
                            e.target.style.display = 'none';
                        }}
                        onLoad={(e) => {
                            console.log('Banner loaded successfully:', data.publicScreenBanners[currentBannerIndex]);
            ...
            >
                {donationAmounts.map((rank, index) => {
                    ...
                    console.log(`🔵 Rendering rank ${index}:`, { 
                        name: rank.name, 
                        amount: rank.amount, 
                        hasImage: !!rank.image,
                        imageUrl: rank.image 
                    ...
                    <button
                        ...
                        onClick={() => setSelectedAmount(rank.amount)}
                    ...
                    >
                        {rank.image ? (
                            ...
                            <img 
                                ...
                                onLoad={(e) => {
                                    console.log(`✅ Image loaded successfully: ${rank.image}`);
                                }}
                                onError={(e) => {
                                    console.error(`❌ Image failed to load: ${rank.image}`);
                                    ...
                                    <div className={styles.liquidGauge}>
                                        <svg viewBox="0 0 200 200" className={styles.gaugeSvg}>
                                            <defs>
                                                ...
                                                {/* Gradient for liquid - using custom color */}
                                                <linearGradient id="liquidGradient" x1="0%" y1="0%" x2="0%" y2="100%">
                                                    <stop offset="0%" stopColor={data?.publicScreenRanksBackgroundColor || '#b45309'} />
                                                    <stop offset="100%" stopColor={data?.publicScreenRanksBackgroundColor || '#b45309'} stopOpacity="0.8" />
                                                </linearGradient>
                                            </defs>
                                            ...
                                            <g clipPath="url(#circleClip)">
                                                <path
                                                    d={`M -50 ${200 - (progressPercent * 1.7)} Q -25 ${195 - (progressPercent * 1.7)}, 0 ${200 - (progressPercent * 1.7)} T 50 ${200 - (progressPercent * 1.7)} T 100 ${200 - (progressPercent * 1.7)} T 150 ${200 - (progressPercent * 1.7)} T 200 ${200 - (progressPercent * 1.7)} T 250 ${200 - (progressPercent * 1.7)} V 250 H -50 Z`}
                ...
                <div className={styles.actionButtons}>
                    <button 
                        ...
                        onClick={() => {
                            ...
                            setInitialDonationAmount(selectedAmount);
                            setIsDonationFormOpen(true);
                        }}
                    >
                        {t('donate')} {selectedAmount ? selectedAmount.toLocaleString() : '0'} {data?.campaign?.currency || '₪'}
                    </button>
            ...
            <section className={styles.tabsSection}>
                <div className={styles.tabsHeader}>
                    ...
                    >
                        <span className={styles.tabLabel}>{t('recent')}</span>
                    </button>
                </div>
                ...
                <div className={styles.tabsContent}>
                    ...
                    {selectedFundraiser && activeTab === 'fundraisers' && (() => {
                        const progressPercentage = selectedFundraiser.targetAmount > 0 
                            ? (selectedFundraiser.totalRaised / selectedFundraiser.targetAmount) * 100
                            : 0;
                                ...
                                <div className={styles.fundraiserDetailsContent}>
                                    <div className={styles.fundraiserDetailsTop}>
                                        ...
                                        <div className={styles.fundraiserDetailsAmount}>
                                            {formatCurrency(selectedFundraiser.totalRaised)}
                                        </div>
                                    </div>
                                                ...
                                                <div style={{
                                                    ...
                                                    {showMonthlyInfo && (
                                                        <div style={{ 
                                                            ...
                                                            {t('monthly')}: {formatCurrency(donor.monthlyAmount || donor.totalAmount)}
                                                        </div>
                                                    )}
                                                </div>
                                            </div>
                            ...
                            return (
                                ...
                                if (item.type === 'fundraiser') {
                                    ...
                                    const progressPercentage = item.targetAmount > 0 
                                        ? (item.totalRaised / item.targetAmount) * 100
                                        : 0;
                                                ...
                                                {data?.campaign?.donationType === 'monthly' ? (
                                                    <div style={{
                                                        ...
                                                        <div style={{ fontSize: '1.15rem', fontWeight: '700', color: '#1e293b', textAlign: 'center' }}>
                                                            {t('total')}: {formatCurrency(item.totalRaised * 12)}
                                                        </div>
                                                        <div style={{ fontSize: '0.75rem', color: '#666', whiteSpace: 'nowrap' }}>
                                                            {t('monthly')}: {formatCurrency(item.totalRaised)}
                                                        </div>
                                                    </div>
                                                ) : (
                                                    <div style={{
                                            ...
                                            <div style={{
                                                ...
                                                {showMonthlyInfo && (
                                                    <div style={{ 
                                                        ...
                                                        {t('monthly')}: {formatCurrency(item.amount)}
                                                    </div>
                                                )}
                                            </div>
                                        </div>
